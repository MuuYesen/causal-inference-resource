...
selected_index = []
for i in [2]:  # 选择连通图2，包含所有的告警类型（事件）
...
if data_id == 1 or data_id == 2:
    ...
    X_tensor = X[:5000]
print('划分后的数据：', len(X['event'].unique()), len(X_tensor['event'].unique()))
...
for tau in range(tau_max + 1):  # 绘制不同时间步的因果效应
...
sns.heatmap((parcorr_result['p_matrix'] < p_th).sum(axis=2) >= 0.7 * (tau_max + 1),  # 绘制总因果效应
            linewidths=0.01,
            vmax=1.0,
            vmin=-1.0,
            cmap='rainbow')
