    ...
    # IV分箱
        ...
        if pd.isnull(total_matrix[0]) or pd.isnull(total_matrix[1]) or total_matrix[0] == 0 or total_matrix[1] == 0:
            ...
            for i in range(bin_num):
                ...
                IV = IV + weight * np.log(
                    (good_bad_matrix[i][1] * total_matrix[0]) / (good_bad_matrix[i][0] * total_matrix[1]))
            M_value = IV
    # 信息熵分箱
    elif method == 3:
        ##总的信息熵
...
def cal_bin_value(x, y, bin_min_num_0=10):
    ...
    df_temp = df_temp.assign(total=lambda x: x['good'] + x['bad'], bin=1, var_name=df_temp.index).assign(
        bad_rate=lambda x: x['bad'] / x['total'])
...
if __name__ == '__main__':
    ...
    data_test1, gain_value_save1, gain_rate_save1 = cont_var_bin(data.Age, data.Survived,
                                                                 ...
                                                                 bin_min_num=20)
    print(cont_var_bin_map(data.Age, data_test1))
    ...
    data_test1, gain_value_save1, gain_rate_save1, del_key_1 = disc_var_bin(data.Cabin, data.Survived,
                                                                            method=1, mmin=3, mmax=12, stop_limit=0.1,
                                                                            bin_min_num=5)
